...
const containerElement = document.querySelector(`.board__tasks`);
const serverData = [
  {
    id: 1,
    color: `black`,
  ...
  },
  {
    ...
    color: `blue`,
    text: ``,
    dateDeadline: {date: `23 september`, time: `11:15pm`},
    hashtags: [`#repeat`, `#cinema`, `#entertaiment`],
    deadline: true
  },
  {
...
const cardElements = getCards(serverData);
...
function createCardElement(task) {
  const template = createCardTemplate(task);
  return createElement(template);
}

function getCards(data) {
  const preparedData = data.map((task) => prepareDataForTemplate(task));
  const cardsFragment = document.createDocumentFragment();
  preparedData.forEach((task) => {
    cardsFragment.appendChild(createCardElement(task));
  });
...
function render(elements) {
  ...
  containerElement.appendChild(elements);
}

function createCardTemplate(data) {
  return `<article class="card card--${data.color} card--repeat">
